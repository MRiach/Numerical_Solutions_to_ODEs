%IRK applied to nonlinear BVP in exercise 4

%m denotes the number of iterations used in the Newton method


function numsol = BVPIRKq4(T,L,dx,dt,v,m)
...
numsol(:,1) = cos(2*pi*X/10).*sin(10*pi*X/10);
...
for i = 2:N+1
   ...
   u = numsol(:,i-1);
   k1(1) = 0.1/(dx^2)*(2*u(2)-2*u(1))+u(1)^2*(1-u(1));
   k1(M+1) = 0.1/(dx^2)*(2*u(M)-2*u(M+1))+u(M+1)^2*(1-u(M+1));
   for j = 2:M
       k1(j) = 0.1/(dx^2)*(u(j+1)-2*u(j)+u(j-1))-v/(2*dx)*(u(j+1)-u(j-1))...
           +u(j)^2*(1-u(j));       
   end    
   ...
   k2(1) = 0.1/(dx^2)*(2*u(2)-2*u(1))+u(1)^2*(1-u(1));
   k2(M+1) = 0.1/(dx^2)*(2*u(M)-2*u(M+1))+u(M+1)^2*(1-u(M+1));
   for j = 2:M
       k2(j) = 0.1/(dx^2)*(u(j+1)-2*u(j)+u(j-1))-v/(2*dx)*(u(j+1)-u(j-1))...
           +u(j)^2*(1-u(j));       
   end  
   ...
   k3(1) = 0.1/(dx^2)*(2*u(2)-2*u(1))+u(1)^2*(1-u(1));
   k3(M+1) = 0.1/(dx^2)*(2*u(M)-2*u(M+1))+u(M+1)^2*(1-u(M+1));
   for j = 2:M
       k3(j) = 0.1/(dx^2)*(u(j+1)-2*u(j)+u(j-1))-v/(2*dx)*(u(j+1)-u(j-1))...
           +u(j)^2*(1-u(j));       
   end  
   ...
   k4(1) = 0.1/(dx^2)*(2*u(2)-2*u(1))+u(1)^2*(1-u(1));
   k4(M+1) = 0.1/(dx^2)*(2*u(M)-2*u(M+1))+u(M+1)^2*(1-u(M+1));
   for j = 2:M
       k4(j) = 0.1/(dx^2)*(u(j+1)-2*u(j)+u(j-1))-v/(2*dx)*(u(j+1)-u(j-1))...
           +u(j)^2*(1-u(j));       
   end
   ...
   F(1) = 0.1/(dx^2)*(2*u(2)-2*u(1))+u(1)^2*(1-u(1));
   F(M+1) = 0.1/(dx^2)*(2*u(M)-2*u(M+1))+u(M+1)^2*(1-u(M+1));
   for j = 2:M
       F(j) = 0.1/(dx^2)*(u(j+1)-2*u(j)+u(j-1))-v/(2*dx)*(u(j+1)-u(j-1))...
           +u(j)^2*(1-u(j));       
   end 
   F = k4-F;
